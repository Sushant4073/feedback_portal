...
aws --endpoint-url=http://127.0.0.1:4566 lambda update-function-configuration --function-name feedback-router --environment "Variables={AWS_ENDPOINT_URL=http://localstack-main:4566,AWS_REGION=us-west-2,DB_URL=postgres://postgres:postgres@postgres:5432/feedback_db?sslmode=disable,S3_BUCKET=feedback-attachments}"
