...
import { getFeedback, getComments, createComment, deleteComment, postVote, createFeedback, updateFeedback, uploadAttachment, listAttachments, getAttachmentDownloadURL, deleteAttachment, getPresignedUploadURL, uploadFileDirectly, confirmAttachmentUpload } from "../services/api";
...
const FeedbackDetails = ({ mode = "view" }) => {
  ...
  const handleCreateFeedback = async () => {
    ...
    try {
      ...
      // If there are pending files, upload them after feedback is created
      ...
      setSubmitting(false);
      alert("Feedback created successfully" + (pendingFiles.length > 0 ? ` with ${pendingFiles.length} file(s)` : "") + "!");
      setTimeout(() => {
        window.location.href = "/";
      }, 100);
    } catch (err) {
