...
export async function getAttachmentDownloadURL(attachmentId) {
  ...
  console.log("Download URL returned:", response.download_url);
...
/**
 * Finalize feedback - sends event to create Jira ticket with all attachments
 * Called after all attachments are uploaded
 */
export async function finalizeFeedback(feedbackId) {
  return fetchAPI(`${API_BASE_URL}/feedback/${feedbackId}/finalize`, {
    method: 'POST',
  });
}
